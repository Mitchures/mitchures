...
const Mitchures = () => {
  const getColors = (isDarkMode) => ({
    primary: isDarkMode ? "#fbc2eb" : "#f6d365",
    secondary: isDarkMode ? "#a6c1ee" : "#fda085",
  })

  const [colors, setColors] = useState(() => {
    if (typeof window !== "undefined") {
      const darkModeMediaQuery = window.matchMedia("(prefers-color-scheme: dark)")
      return getColors(darkModeMediaQuery.matches)
    }
    return getColors(false) // Default to light mode during SSR
  })

  useEffect(() => {
    if (typeof window !== "undefined") {
      const darkModeMediaQuery = window.matchMedia("(prefers-color-scheme: dark)")
      const updateColors = (event) => setColors(getColors(event.matches))

      darkModeMediaQuery.addEventListener("change", updateColors)

      return () => {
        darkModeMediaQuery.removeEventListener("change", updateColors)
      }
    }
  }, [])
